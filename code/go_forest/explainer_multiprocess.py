...
class GOFRExplainer():
    ...
    def explain_hierarchy(self, x_gene, importance_type, importance_method, threshold):
        ...
        def update_dict(nodes_imptc_df: DataFrame):
            for term_name, importance in nodes_imptc_df.iterrows():
                if "GO" in term_name:
                    imptc_dict[term_name] += float(importance)
    ...
    def explain_genes(
        ...
        def update_dict(gene_imptc_df: DataFrame):
            for index, importance in gene_imptc_df.iterrows():
                if "gene" in index:
                    gene_imptc_dict[index] += float(importance)
    ...
    def explain_samples_gene_multi_process(self, x_gene, importance_type):
        """获取预测数据集中每一个样本的特征重要性
        ...
        manager = Manager()
        gene_queue = manager.Queue(1)
        gene_queue.put(pd.DataFrame(np.zeros((n_smaples, n_genes)), columns=gene_feature_names))
        queue_weight = manager.Queue(1)
        queue_weight.put(pd.DataFrame(np.zeros((n_smaples, n_genes)), columns=gene_feature_names))
        # 定义子进程函数
        def sub_process(layer_id, layer,):
            """逐层解释的子线程函数"""
            start_time = time.time()
            for term_name in layer:
                if term_name == "GO:0008150":
                    print(term_name)
                x = self._build_node_input(term_name, x_gene, nob_dict, lcb_array)
                node_weight = float(weights.loc[term_name])
                # 选择解释方案
                if importance_type == 'shap':
                    # imptc_df = self._explain_predict_input_multi_process(x, term_name)    # 多进程
                ...
                mask_df = gene_queue.get()
                mask_df.loc[:, gene_imptc_df.columns] = gene_imptc_df
                gene_queue.put(mask_df)
                mask_weight_df  = queue_weight.get()
                mask_weight_df.loc[:, gene_imptc_df.columns] = gene_imptc_df * node_weight
                queue_weight.put(mask_weight_df)
                # print(mask_weight_df.sum())
            end_time = time.time()
            print(f"layer{layer_id} finished, elapsed {end_time-start_time:.2f}s")
        ...
        weights = self.model.weighter.weights * 1092
        # 多线程解释Layer
        process_list = [Process(target=sub_process, args=(layer_id, layer, ))\
            for layer_id, layer in zip(np.arange(6)[::-1], self.term_layer_list[::-1])]
        for process in process_list:
            process.start()
            # break
        ...
        importance_df = gene_queue.get()
        importance_weight_df = queue_weight.get()
    ...
    def explain_samples_gene_single_process(self, x_gene, importance_type):
        ...
        for layer_id, layer in zip(np.arange(6)[::-1], self.term_layer_list[::-1]):
            ...
            for term_name in layer:
                ...
                unweight_df.loc[:, gene_imptc_df.columns] = gene_imptc_df
    ...
    def explain_samples_term_multi_process(self, x_gene, importance_type):
        """获取预测数据集中每一个样本决策过程中的增强特征的特征重要性
        ...
        import time
        from multiprocessing import Process, Manager, Queue
        # 初始化结果矩阵: 存储n个样本, 1091个Term 使用Queue在多个子进程之间共享
        term_feature_names = [
        ...
        manager = Manager()
        term_queue = manager.Queue(1)
        term_queue.put(pd.DataFrame(np.zeros((n_smaples, n_terms)), columns=term_feature_names))
        queue_weight = manager.Queue(1)
        queue_weight.put(pd.DataFrame(np.zeros((n_smaples, n_terms)), columns=term_feature_names))
        # 定义子进程函数
        def sub_process(layer_id, layer,):
            """逐层解释的子线程函数"""
            start_time = time.time()
            for term_name in layer:
                ...
                term_imptc_df = imptc_df.loc[:, all_term_columns]  # 仅保留gene的特征重要性
                mask_df = term_queue.get()
                mask_df.loc[:, term_imptc_df.columns] = term_imptc_df
                term_queue.put(mask_df)
                mask_weight_df  = queue_weight.get()
                mask_weight_df.loc[:, term_imptc_df.columns] = term_imptc_df * node_weight
                queue_weight.put(mask_weight_df)
                # print(mask_weight_df.sum())
            end_time = time.time()
            print(f"layer{layer_id} finished, elapsed {end_time-start_time:.2f}s")
    ...
    def explain_samples_term_single_process(self, x_gene, importance_type):
        ...
        for layer_id, layer in zip(np.arange(6)[::-1], self.term_layer_list[::-1]):
            ...
            for term_name in layer:
                ...
                unweight_df.loc[:, term_imptc_df.columns] = term_imptc_df
    ...
    def explain_node(self, x_gene, term_name):
        ...
        nob_dict, lcb_array = self._get_bc(x_gene)
        x = self._build_node_input(term_name, x_gene, nob_dict, lcb_array)
        # return self._explain_predict_input_multi_process(x, term_name).T
    ...
    def _explain_predict_input_multi_process(self, x, term_name):
        """多线程解释单个节点中输入样本的特征重要性
        ...
        from multiprocessing import Process, Manager, Queue
        manager = Manager()
        explanations = manager.list([[]]*5)     # shape=(n_base_learners, )
        def sub_process(forest_id, explainations):
            model = node.estimators[forest_id]
            # make explainer
            explainer = shap.explainers.Tree(
                model=model, 
                # data=x,
                feature_perturbation="tree_path_dependent", 
                # tree_path_dependent 不可以在多进程下使用
                feature_names=feature_names,
            )
            explainations[forest_id] = explainer(x)

        process_list = [Process(target=sub_process, args=(i, explanations, )) 
        ...
        for process in process_list:
            ...
            break
        # 取平均值
        explanations = list(explanations)   # explainations's shape=(n_base_learner, n_samples, n_features)
        shapley_values_ave = np.mean(
            [explaintion.values for explaintion in list(explanations)],
            axis=0,
        )    # explaintion's shape=(n_samples, n_features)
        ...
        # importance_df = pd.DataFrame(node_explaintion.values, columns=node_explaintion.feature_names)
        importance = self._calcul_feature_importance_multi_samples(node_explaintion)
        importance_df = pd.DataFrame(importance, columns=node_explaintion.feature_names)
...
class GOFCLFExplainer(GOFRExplainer): 
    ...
    def _explain_nodes_multi_process(self, term_name, x):
        ...
        def get_shap(forest_id, res_dict):
            ...
            explainer = shap.explainers.Tree(model, x, feature_names=feature_names)
            # get shap
            shap_value = explainer(x,  check_additivity=False )
            res_dict[str(forest_id)] = shap_value
    ...
    def explain_hierarchy(self, x_gene, importance_type, importance_method, threshold):
        ...
        # 将孩子节点的数量作为重要性分数的影响因子
        for term_name, children_list in term_child_map.items():
            weighted_imptc_dict[term_name] *= (len(children_list)+1)
    ...
    def _explain_predict_input_single_process(self, x, term_name):
        ...
        -------
        importance_df: DataFrame, shape=(n_samples, n_features)"""
        importance_df = super()._explain_predict_input_single_process(x, term_name)
        importance_df = importance_df.T     # 转化shape为(n_features, n_samples)
        node: NodeCLF = self.model._get_node(term_name)
        importance_df = self._merge_term_class_importance(importance_df, node.child_list)
        return importance_df.T      # 转化shape为(n_samples, n_features)
